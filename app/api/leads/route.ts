...
export async function DELETE(request:Request){
  ...
  return Response.json({deleted:true,enqNo,deletedVisitForms:Number(results[0]?.meta?.changes ?? 0),deletedProposals:Number(results[1]?.meta?.changes ?? 0)});
}
